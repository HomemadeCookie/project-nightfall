...
The basemap is cut out of a Protomaps planet build by the pipeline job itself and cached for a month, because `pmtiles extract` reads only the byte ranges it needs: the Philippine extract is about 20 MB and four seconds of work against a 137 GB source, so it needs no schedule, no archive, and no credential of its own. Locally, `pmtiles` must be on the `PATH` for `pipeline/scripts/build_basemap.py` to have anything to run; without it the app draws positions on a plain background and says so, since the mobility data is the product and the basemap is context.

One other absence is deliberate rather than broken: individual tracks are drawn from zoom 9 up, and the area summaries that belong to wider views are a later phase, so below that zoom the app states that rather than showing an empty map.
